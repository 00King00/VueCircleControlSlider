export default class CircleState {
    ...
    converAngleToValue(angle) {
        return angle >= 358 ? this.lastStep : this.stepsArray[Math.floor(angle / this.angleUnit)];
    }

    getPositionX(angle) {
        return this.center + this.radius * Math.cos((angle + this.offset) * this.convertedDegreesToRadians) - 0.00001;
    }

    getPositionY(angle) {
        return this.center + this.radius * Math.sin((angle + this.offset) * this.convertedDegreesToRadians) - 0.00001;
    }
}
